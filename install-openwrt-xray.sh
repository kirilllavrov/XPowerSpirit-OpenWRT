#!/bin/sh
# install-openwrt-xray-fixed.sh — полностью исправленная версия
...
# 1. Подписка
printf "Введите URL подписки VLESS (или нажми Enter, чтобы оставить текущий): "
read -r SUB_URL_INPUT
if [ -n "$SUB_URL_INPUT" ]; then
    echo "$SUB_URL_INPUT" > "$SUB_FILE"
fi
[ ! -s "$SUB_FILE" ] && { echo "Ошибка: нет URL подписки"; exit 1; }
...
# 5. nftables TProxy (ruleset-post + защита от дублирования)
...
# 6. Policy routing + persistency после перезагрузки
...
curl -s -L -m 20 -H "User-Agent: Happ" -H "x-hwid: $HWID" "$(cat "$SUB_FILE")" \
...
echo
echo "=== ДИАГНОСТИКА СРАЗУ ПОСЛЕ УСТАНОВКИ ==="
"$DIAG"

echo "=== Установка завершена ==="
echo "HWID: $HWID"
echo "Если порт 12345 всё ещё не слушается — выполни: /etc/init.d/xray restart && netstat -tulnp | grep 12345"