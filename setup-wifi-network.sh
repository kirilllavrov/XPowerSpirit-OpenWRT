...
# === 3. Проверка DHCP Guest ===
echo "Проверка DHCP Guest..."
if ! uci show dhcp.$GUEST_NET >/dev/null 2>&1; then
    echo "⚠️ DHCP для $GUEST_NET не найден — создайте его через install-openwrt-xray.sh"
else
    echo "✅ DHCP для $GUEST_NET уже настроен"
fi

# === 4. Firewall Guest ===
echo "Проверка Firewall Guest..."
if ! uci show firewall | grep -q "zone.*name='$GUEST_NET'"; then
    echo "⚠️ Зона $GUEST_NET не найдена, создайте её через install-openwrt-xray.sh"
fi
