...
# WPA2 + WPA3 (sae-mixed) + Auto channel + Очистка старых конфигураций
...
# Сохраняем только базовые настройки радио (wifi-device)
...
for RADIO in $(uci show wireless | sed -n 's/^\(wireless\.\([^=]*\)\)=wifi-device.*/\2/p'); do
    ...
    uci set wireless.home_${RADIO}.encryption="sae-mixed"   # WPA2 + WPA3
...
for RADIO in $(uci show wireless | sed -n 's/^\(wireless\.\([^=]*\)\)=wifi-device.*/\2/p'); do
    ...
    uci set wireless.guest_${RADIO}.encryption="sae-mixed"   # WPA2 + WPA3
